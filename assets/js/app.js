var helpers = {
    ...
    formatDate: function (item) {
        var momentInstance = moment.tz(item["date"], "MM-DD-YYYY hh:mma", window["locations"][item["location"]]["timeZone"]);
...
var template = {
    date: function (item) {
        ...
        var ct = new Date();
        if (item["date"]["unix"] <= moment().utc().unix()) {
            rendered += " past"
        }
